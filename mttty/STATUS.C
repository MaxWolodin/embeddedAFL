...
void UpdateStatus(char * szText)
{
    ...
    //if (READCALLBACK(TTYInfo))
    //{
    //    READCALLBACK(TTYInfo)(szNewMsg, _tcslen(szNewMsg));
    //}
    //else
    //{
    //    _tprintf(szNewMsg);
    ...
    char* szFormat = "StatusReporter %d: %s";    // format for wsprintf
    ...
    */
    int ilenFinal = _tcslen(szText) +  30;
    szFinal = (char*)LocalAlloc(LPTR, ilenFinal * sizeof(char));
    ...
    if (szFinal == NULL)	// if no final buffer, then can't format error
        ...
        int nwritten = StringCchPrintfA(szFinal, ilenFinal, szFormat, dwMessageCounter, szText);

        if (STATUSCALLBACK(TTYInfo))
        {
            STATUSCALLBACK(TTYInfo)(szText, _tcslen(szText)*2);
        }
        else
        {
            _tprintf(szFinal);
