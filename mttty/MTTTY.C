...
/* ADDED SECTION for embeddedAFL
*/
#define DLL __declspec(dllexport)
typedef char* (__stdcall* GetFilePathCallback)(char* filter);

DLL void ConnectToSerialPort(ProgressCallback readCallback, ProgressCallback statusCallback, ProgressCallback errorCallback);
...
int main(int argc, char* args[]) {
	UpdateStatus("START");
	ConnectToSerialPort(0,0,0);
	return 0;
}

void ConnectToSerialPort(ProgressCallback readCallback, ProgressCallback statusCallback, ProgressCallback errorCallback)
{
	MSG msg;
	if (!InitializeApp(0, 0))
	{
		return 10; // MTTTY could not start!
	}
	UpdateStatus("APP initialized");
		
	_tcscpy_s(gszPort, 10, _T("COM7"));
	InitTTYInfo();
	READCALLBACK(TTYInfo) = readCallback;
	STATUSCALLBACK(TTYInfo) = statusCallback;
	ERRORCALLBACK(TTYInfo) = errorCallback;
	ClearCommError(COMDEV(TTYInfo), 0, 0);

	UpdateStatus("MTTTY initialized");
	SetupCommPort();
	//*info = TTYInfo;
