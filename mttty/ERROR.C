...
{
	char*szFormat = "ErrorReporter %d: %s.\n%s\n";    // format for wsprintf
	...
	*/
	...
	if (szFinal == NULL)	// if no final buffer, then can't format error
		...
		int nwritten = StringCchPrintfA(szFinal, ilenFinal, szFormat, dwErr, szMessage, szExtended);

		if (ERRORCALLBACK(TTYInfo))
		{
			ERRORCALLBACK(TTYInfo)(szFinal, ilenFinal);
		}
		else
		{
			_tprintf(szFinal);
