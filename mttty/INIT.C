...
BOOL InitTTYInfo()
{
	...
	RTSCONTROL(TTYInfo) = RTS_CONTROL_HANDSHAKE;//RTS_CONTROL_ENABLE;
	XONCHAR(TTYInfo) = ASCII_XON;
	XOFFCHAR(TTYInfo) = ASCII_XOFF;
	XONLIMIT(TTYInfo) = 0;
	XOFFLIMIT(TTYInfo) = 0;
	CTSOUTFLOW(TTYInfo) = TRUE; //FALSE;
...
void StartThreads(void)
{
	...
	{
		UpdateStatus("READSTATTHREAD started");
	}
	...
	{
		UpdateStatus("WRITERTHREAD started");
	}
...
HANDLE SetupCommPort()
{
	...
	UpdateStatus("Start SetupCommPort");
	//
	...
	if (COMDEV(TTYInfo) == INVALID_HANDLE_VALUE) {
		...
		UpdateStatus("Communication port handle opened");
	...
	UpdateStatus("Comm buffer sizes set");
